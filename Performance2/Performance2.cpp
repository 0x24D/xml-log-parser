...
auto calculateDurationsAndAverage(concurrency::concurrent_queue<LogItem>& logData, concurrency::concurrent_queue<pair<string, int>>& durations, float& averageDuration) {
    auto i = 0;
    auto total = 0;
    while (true) {
        LogItem item;
...
auto constructDurationsJson(concurrency::concurrent_queue<pair<string, int>>& durations, float& averageDuration, concurrency::concurrent_queue<string>& durationsJson) {
...
auto outputStatistics(concurrency::concurrent_queue<string>& viewsJson, concurrency::concurrent_queue<string>& durationsJson, const string& fileName) {
    ofstream jsonFile(fileName);
    jsonFile << "{\n";
    string startWith;
    auto firstValue = true;
...
auto processLines(concurrency::concurrent_queue<string>& unProcessedLines) {
    ...
    thread tOutputStats(outputStatistics, ref(viewsJson), ref(durationsJson), "statistics.json");
